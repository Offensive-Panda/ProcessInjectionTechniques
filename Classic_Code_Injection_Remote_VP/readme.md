...
3. `Allocate Memory in the Target Process:` `VirtualAllocEx` function to allocate `RW` memory in the target process's address space.
4. `Write Shellcode to the Allocated Memory:` `WriteProcessMemory` to write the shellcode into the allocated memory.
5. `Chnage the memory protection:` `VirtualProtect` to change the RW protection to RX.
...
my_sc_mem = VirtualAllocEx(hProcess, 0, sc_len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
```
...
```
`VirtualProtect` is to change the memory protection from RW to RX before executing the shellcode.
```cpp
// change the allocated memory section permission to Executable
VirtualProtectEx(hProcess, my_sc_mem, sc_len, PAGE_EXECUTE_READ, &oldprotect);
```
...
For GitHub-Repo Click Here: [Offensive-Panda/ProcessInjectionTechniques](https://github.com/Offensive-Panda/ProcessInjectionTechniques/tree/main/Classic_Code_Injection_Local/Classic_Code_Injection_Remote_VP)
