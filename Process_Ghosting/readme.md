<h1 align="center">Process Ghosting</h1>
<p align="center">
  <img src="../Assets/PG_AI.jpg" alt="UH" width="500px">
</p>
...
## Steps
1. Load the malicious executable into memory.
2. Mark the file for deletion and create a section from it.
3. Launch the process from the section.
4. Calculate the entry point address.
5. Execute the process in memory using a remote thread.


## Walkthrough

Reads an executable file into memory.
* `CreateFileW:` Opens the file injected.exe for reading.
* `GetFileSize:` Retrieves the size of the file.
* `VirtualAlloc:` Allocates a buffer in memory to store the file content.
* `ReadFile:` Reads the file into the allocated buffer.
...
```
Creates a section object from a file marked for deletion, ensuring the file is removed from disk but the process can still use it.
* `NtOpenFile:` Opens the file with read, write, and delete access.
* `NtSetInformationFile:` Marks the file for deletion after it’s closed.
* `WriteFile:` Writes the data buffer into the file.
* `NtCreateSection:` Creates a section object from the file (a file-backed memory region).
...
[Blog Post](https://whokilleddb.github.io/blogs/)

## Demonstration

![](Asset/PE.gif)

For GitHub-Repo Click Here: [Offensive-Panda/ProcessInjectionTechniques](https://github.com/Offensive-Panda/ProcessInjectionTechniques/tree/main/Process_Ghosting/Process_Ghosting)
