<h1 align="center">Module Stomping</h1>
<p align="center">
  <img src="../Assets/MS_AI.jpg" alt="UH" width="500px">
</p>
...
## Steps
1. Getting the Target Process ID
2. Finding the Base Address of a Loaded Module
3. Allocating Memory in the Target Process
4. Writing DLL Path to the Allocated Memory:
5. Creating Remote Thread to Load the DLL:
6. Finding the Base Address of the Injected DLL:
7. Overwriting the DLL's Entry Point with Shellcode:
8. Executing the Shellcode:
...
```
This function finds the base address of a specific module (DLL) within a process by iterating through the loaded modules of the target process.
...
```
Allocates memory in the target process (VirtualAllocEx) for storing the path to the DLL to be injected and write the path of DLL.
```cpp
...
```
Overwriting the DLL's Entry Point with Shellcode and execute it.
```cpp
...
For GitHub-Repo Click Here: [Offensive-Panda/ProcessInjectionTechniques](https://github.com/Offensive-Panda/ProcessInjectionTechniques/tree/main/Module_Stomping/Module_Stomping)
